...
def engineer_features(df, is_training=True, scaler_path='models/scaler.pkl', config_path='config/config.yaml'):
    ...
    if is_training:
        ...
        to_scale = [c for c in numerical_features if c in df.columns]
        missing = set(numerical_features) - set(to_scale)
        if missing:
            logger.warning(f"Numerical features missing at training time and skipped: {missing}")
        ...
        to_scale = [c for c in numerical_features if c in df.columns]
        extra_new = set(to_scale) - set(getattr(scaler, 'feature_names_in_', to_scale))
        if extra_new:
            logger.warning(f"Dropping unseen numerical features for inference: {extra_new}")
            to_scale = [c for c in to_scale if c not in extra_new]
